...
export default function UndergroundReports() {
  const [loading, setLoading] = useState(true);
  const [overview, setOverview] = useState(null);
  const [error, setError] = useState("");
  const [devEuiFilter, setDevEuiFilter] = useState("");
  const [days, setDays] = useState(3); // últimos 3 días
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const data = await fetchUndergroundOverview(params);
  ...
  if (loading) {
    return <div style={{ padding: 24 }}>Cargando reporte de soterrado...</div>;
  }
  ...
  return (
    <div style={{ maxWidth: 1200, margin: "24px auto", padding: "0 16px" }}>
      <header style={{ marginBottom: 16 }}>
        <h1 style={{ marginBottom: 4 }}>Reportes de Sensores Soterrados</h1>
        <p style={{ margin: 0, color: "#4b5563" }}>
          Análisis histórico de distancia promedio en ventanas de 1 minuto.
        </p>
      </header>
      ...
      >
        ...
        <p style={{ marginTop: 0, fontSize: 13, color: "#6b7280" }}>
          Distancia promedio (cm) por ventana de 1 minuto.
        </p>
      ...
      >
        ...
        <p style={{ marginTop: 0, fontSize: 13, color: "#6b7280" }}>
          Sensores con mayor distancia promedio medida.
        </p>
        ...
        <div style={{ width: "100%", height: 320 }}>
          <ResponsiveContainer>
            <BarChart data={porSensor}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="distance_mean" name="Distancia promedio (cm)" />
              <Bar dataKey="distance_max" name="Distancia máxima (cm)" />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </section>
      ...
      >
        ...
        <p style={{ marginTop: 0, fontSize: 13, color: "#6b7280" }}>
          Conteo de ventanas por rango de distancia.
        </p>
        ...
        <div style={{ width: "100%", height: 260 }}>
          <ResponsiveContainer>
            <BarChart data={histograma}>
              ...
              <XAxis
                dataKey="bin_start"
                tickFormatter={(v) => `${v}-${v + 19} cm`}
              />
              <YAxis />
              <Tooltip
                formatter={(value, _name, props) => [
                  value,
                  `Conteo (${props.payload.bin_start}-${props.payload.bin_end} cm)`,
                ]}
              />
